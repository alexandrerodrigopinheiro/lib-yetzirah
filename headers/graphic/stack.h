#ifndef YGL_GRAPHIC_STACK_H_
#define YGL_GRAPHIC_STACK_H_
...
namespace ygl
{
	namespace graphic
	{
		...
		public:
			...
			stack(const ygl::graphic::stack&) = delete;

			stack(const std::string& filename);
			...
			void draw(const ygl::math::pointd& position, int spot = -1, const ygl::math::rotated& rotate = ygl::math::rotated().null(), const ygl::math::sized& scale = ygl::math::sized().null(), const ygl::math::colord& color = ygl::math::colord().null());
		...
		protected:
			...
			std::size_t										_spot			{ 0 };
			std::vector<uint>								_data			{ std::vector<uint>() };
			...
			std::vector<ygl::math::sized>					_size			{ std::vector<ygl::math::sized>() };
