#ifndef YGL_GRAPHIC_SPRITE_H_
#define YGL_GRAPHIC_SPRITE_H_
	...
	namespace graphic
	{
		class sprite
		{
			public:
				...
				sprite(const ygl::graphic::sprite&) = delete;

				sprite(const std::string& filename, std::size_t colunms, std::size_t frames);
				...
				void draw(const ygl::math::pointd& position, int spot = -1, const ygl::math::rotated& rotation = ygl::math::rotated().null(), const ygl::math::sized& scale = ygl::math::sized().null(), const ygl::math::colord& color = ygl::math::colord().null());
